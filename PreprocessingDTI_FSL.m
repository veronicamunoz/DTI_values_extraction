...
if ~isempty(B0dir);
    ...
    v1 = spm_vol('b0_P.nii');
    if mod(v1.dim(3),2)==0
        status=system([FSLcommand 'topup --imain=b0_PA.nii --datain=acqparams.txt --config=/media/veronica/DATAPART1/Donnees/DTIPark/b02b0.cnf --out=topup_results']);
    else
        status=system([FSLcommand 'topup --imain=b0_PA.nii --datain=acqparams.txt --config=/media/veronica/DATAPART1/Donnees/DTIPark/b02b02.cnf --out=topup_results']);% b02b02.cnf sans subsampling dans le répertoire contenant tous les centres
    end
    if status==0
